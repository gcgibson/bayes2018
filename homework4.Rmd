...
mu.alpha.hat.df <- data.frame(Legend = c(rep("Posterior",length(mu.alpha.hat)),rep("Prior",10000)),val = c(mu.alpha.hat,rnorm(10000,20,6)))

mu.alpha.hat.df$Legend <- as.factor(mu.alpha.hat.df$Legend)
p1 <- ggplot(data=mu.alpha.hat.df,
       aes(x=val))+
...
sigma.alpha.hat.df <- data.frame(Legend = c(rep("Posterior",length(sigma.alpha.hat)),rep("Prior",10000)),val = c(sigma.alpha.hat,runif(10000,0,5)))

sigma.alpha.hat.df$Legend <- as.factor(sigma.alpha.hat.df$Legend)
p2 <- ggplot(data=sigma.alpha.hat.df,
       aes(x=val))+
...
sigma.y.hat.df <- data.frame(Legend = c(rep("Posterior",length(sigma.y.hat)),rep("Prior",10000)),val = c(sigma.y.hat,runif(10000,0,5)))

sigma.y.hat.df$Legend <- as.factor(sigma.y.hat.df$Legend)
p3 <- ggplot(data=sigma.y.hat.df,
       aes(x=val))+
...
### Problem 2
```{r,echo=FALSE,warning=FALSE,message=FALSE}
print (round(mod0$BUGSoutput$summary[c("mu.alpha","sigma.alpha","sigma.y"), c("n.eff", "Rhat")],3))
...
The high $N_{eff}$ values show that the chains have low auto-covariance, which shows that the chains are able to explore the full parameter space. The $\hat{R}$ value of 1 for all params show that all the chains have mixed well, and they did not get stuck in local modes.
...
plot(c(cty.mns.j-partpooled.j) ~ sample.size.jittered.j, type = "p",
     ...
     xlab = expression(paste(n[j], ", group sample size")), pch = 20,
     log = "x",
     ylim = lims)

abline(h=ybarbar)

dat <- data.frame(x=cty.mns.j, y1=partpooled.j, y2=sample.size.jittered.j)


plot(y1 ~ x, data=dat, ylab = expression(alpha[j]), xlab = "group sample mean",cex=0)
text(cty.mns.j, partpooled.j, labels=round(sample.size.jittered.j,1), cex= 0.7)
abline(0,1)
abline(h=ybarbar)
```

We can see that there is more variability of $\bar{y_j}$ around $\alpha_j$ when sample size is small, which makes sense because the data does not pull $\alpha_j$ towards $\bar{y_j}$. From plot 2, we can see that larger sample sizes are pulled closer to $\bar{y_j}$ and smaller sample sizes are pulled closer to $\bar{\bar{y}}$.
