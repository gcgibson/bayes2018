...
```{r}
srrs2 <- read.table("/Users/gcgibson/Bayes//radon.dat", header=T, sep=",")
#names(srrs2)
#unique(srrs2$state) # states
# we'll use MN
mn <- srrs2$state=="MN"
# Names of variables are slightly different from GH; 
# I prefer coding where the subscript gives info about the dimension,
# whether it is for all obs (houses i=1,2,..,n), counties (j=1,2,..,J).
radon.i <- srrs2$activity[mn]
#summary(radon.i)
#hist(radon.i)
# GH set any zero levels to 0.1 (explained on p... in book)
logradon.i <- log (ifelse (radon.i==0, .1, radon.i))
floor.i <- srrs2$floor[mn]       # 0 for basement, 1 for first floor
n <- length(radon.i)
y.i <- logradon.i
x.i <- floor.i

# get county index variable
county.i <- as.vector(srrs2$county[mn])
